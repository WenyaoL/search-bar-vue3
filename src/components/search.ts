...
class SearchResult{
    ...
    constructor(start:number,end:number,node:Node,matchVal?:RegExpMatchArray){
        this.start = start
        this.end = end
        this.node = node
    ...
    for (let index = 0; index < root.childNodes.length; index++) {
        ...
        if(node.nodeType == 3){
            ...
            while((start = text.indexOf(val,start)) !== -1){
                ...
                let matchResult:SearchResult = new SearchResult(start,end,node)
        ...
        if(node.nodeType == 3){
            ...
            while ((regResult = regex.exec(text)) !== null) {
                let start = regResult.index,
                    end = regResult.index + regResult[0].length
                let matchResult:SearchResult = new SearchResult(start,end,node,regResult)
                result.push(matchResult)
            }
            continue;
        }
...
export function removeHighlightKey(highlightClass:string){
    ...
    for (let index = 0; index < keys.length; index++) {
        ...
        if(key.textContent) key.replaceWith(key.textContent)
