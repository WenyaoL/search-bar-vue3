<template>
  <div class="search-bar" v-show="!hidden">
...
let searchValue = ref('')
let isCaseSensitive = ref(false)
let isWholeWord = ref(false)
let isRegexp = ref(false)
let searchMatches = reactive<{
  index:number,
  result:any
}>({index:-1,result:[]})
...
let next = ()=>{
  ...
  searchMatches.index = (searchMatches.index+1)%searchMatches.result.length
  selectMatch(searchMatches.result,searchMatches.index)
}
...
let previous = ()=>{
  ...
  if(searchMatches.index <= 0){
    searchMatches.index = searchMatches.result.length -1
    selectMatch(searchMatches.result,searchMatches.index)
    return 
  }
...
let search = (text:string)=>{
  let node = document.querySelector(props.root)
  if(!node) return
  //高亮标记
  if(isHighlight.value){
    
    //高亮搜索
    searchMatches.result = highlight(
      node,
      text,
      {
...
let checkoutCaseSensitive = (event:MouseEvent)=>{
  let element = event.currentTarget as HTMLElement
  isCaseSensitive.value = !isCaseSensitive.value
  if(isCaseSensitive.value) element.classList.add("checked")
  else element.classList.remove("checked")
  search(searchValue.value)
}

let checkoutWholeWord = (event:MouseEvent)=>{
  let element = event.currentTarget as HTMLElement
  isWholeWord.value = !isWholeWord.value
  if(isWholeWord.value) element.classList.add("checked")
  else element.classList.remove("checked")
  search(searchValue.value)
}

let checkoutRegex = (event:MouseEvent)=>{
  let element = event.currentTarget as HTMLElement
  isRegexp.value = !isRegexp.value
  if(isRegexp.value) element.classList.add("checked")
  else element.classList.remove("checked")
  search(searchValue.value)
}
</script>
